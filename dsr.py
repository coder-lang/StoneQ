...
# from .doc_intelligence import extract_markdown_from_file # Use the wrapper
# from .chunking import recursive_chunk_markdown # Use the one from functions/chunking.py
# from llm_service import extract_with_llm, merge_json_chunks # Use the provided module
...
#         # Instead of calling LLM here, raise an exception or return a default/fallback prompt string.
#         # Calling LLM here during import/load could cause the error you see.
#         # Option 1: Raise an exception
#         raise FileNotFoundError(f"Prompt file not found: {file_path}")
#         # Option 2: Return a simple fallback (less ideal, but avoids LLM call)
#         # return "Extract the requested details from the context and return a JSON object."
...
#     caf_prompt = load_prompt("caf_prompt.txt") # Load prompt inside the function
#     caf_output = extract_with_llm(caf_prompt, caf_markdown) # This calls the LLM
#     return {"caf": safe_json_parse(caf_output)}
...
#     form1_prompt = load_prompt("form1_prompt.txt") # Load prompt inside the function
#     form1_output = extract_with_llm(form1_prompt, form1_markdown) # This calls the LLM
#     return {"form1": safe_json_parse(form1_output)}
...
#     nabet_prompt = load_prompt("nabet_prompt.txt") # Load prompt inside the function
#     nabet_output = extract_with_llm(nabet_prompt, nabet_markdown) # This calls the LLM
#     return {"nabet": safe_json_parse(nabet_output)}
...
#     cc_prompt = load_prompt("cluster_certificate_prompt.txt") # Load prompt inside the function
#     chunks = recursive_chunk_markdown(str(cc_markdown))
#     outputs = [extract_with_llm(cc_prompt, chunk) for chunk in chunks] # This calls the LLM for each chunk
...
#     ss_prompt = load_prompt("site_survey_prompt.txt") # Load prompt inside the function
#     ss_output = extract_with_llm(ss_prompt, ss_markdown) # This calls the LLM
#     return {"ss": safe_json_parse(ss_output)}

# def extract_emp(emp_file):
#     """Extract EMP document data."""
#     emp_result, emp_markdown = extract_markdown_from_file(emp_file)
#     emp_prompt = load_prompt("emp_prompt.txt") # Load prompt inside the function
#     emp_output = extract_with_llm(emp_prompt, emp_markdown) # This calls the LLM
#     return {"emp": safe_json_parse(emp_output)}
...
# #     emp_result, emp_markdown = extract_markdown_from_file(emp_file)
# #     logger.info(f"Markdown extracted: {len(emp_markdown)} chars")
# #     logger.info(f"Markdown preview: {emp_markdown[:200]}")
    
# #     emp_prompt = load_prompt("emp_prompt.txt")
# #     logger.info(f"Prompt loaded: {len(emp_prompt)} chars")
    
# #     logger.info("Calling LLM...")
# #     emp_output = extract_with_llm(emp_prompt, emp_markdown)
# #     logger.info(f"LLM returned: {type(emp_output)}, length: {len(str(emp_output))}")
# #     logger.info(f"LLM output: {emp_output}")
    
# #     parsed = safe_json_parse(emp_output)
...
#     mpa_prompt = load_prompt("mpa_prompt.txt") # Load prompt inside the function
#     mpa_output = extract_with_llm(mpa_prompt, mpa_markdown) # This calls the LLM
#     return {"mpa": safe_json_parse(mpa_output)}
...
#     nocgp_prompt = load_prompt("nocgp_prompt.txt") # Load prompt inside the function
#     nocgp_output = extract_with_llm(nocgp_prompt, nocgp_markdown) # This calls the LLM
#     return {"nocgp": safe_json_parse(nocgp_output)}
...
#     nocforest_prompt = load_prompt("nocforest_prompt.txt") # Load prompt inside the function
#     nocforest_output = extract_with_llm(nocforest_prompt, nocforest_markdown) # This calls the LLM
#     return {"nocforest": safe_json_parse(nocforest_output)}
...
#     nocgsda_prompt = load_prompt("nocgsda_prompt.txt") # Load prompt inside the function
#     nocgsda_output = extract_with_llm(nocgsda_prompt, nocgsda_markdown) # This calls the LLM
#     return {"nocgsda": safe_json_parse(nocgsda_output)}

# def extract_kprat(kprat_file):
#     """Extract KPRAT document data."""
#     kprat_result, kprat_markdown = extract_markdown_from_file(kprat_file)
#     kprat_prompt = load_prompt("kprat_prompt.txt") # Load prompt inside the function
#     kprat_output = extract_with_llm(kprat_prompt, kprat_markdown) # This calls the LLM
#     return {"kprat": safe_json_parse(kprat_output)}
...
#     gsr_prompt = load_prompt("gsr_prompt.txt") # Load prompt inside the function
#     gsr_output = extract_with_llm(gsr_prompt, gsr_markdown) # This calls the LLM
#     return {"gsr": safe_json_parse(gsr_output)}
...
#     qlp_prompt = load_prompt("qlp_prompt.txt") # Load prompt inside the function
#     qlp_output = extract_with_llm(qlp_prompt, qlp_markdown) # This calls the LLM
#     return {"qlp": safe_json_parse(qlp_output)}

# def extract_od(od_file):
#     """Extract Ownership Document (OD) data."""
#     od_result, od_markdown = extract_markdown_from_file(od_file)
#     od_prompt = load_prompt("od_prompt.txt") # Load prompt inside the function
#     od_output = extract_with_llm(od_prompt, od_markdown) # This calls the LLM
...
#     dsr_prompt = load_prompt("dsr_prompt.txt") # Load prompt inside the function
#     dsr_output = extract_with_llm(dsr_prompt, dsr_markdown) # This calls the LLM
#     return {"dsr": safe_json_parse(dsr_output)}
...
#     regrassing_prompt = load_prompt("regrassing_prompt.txt")  # Load prompt from the prompt folder
#     regrassing_output = extract_with_llm(regrassing_prompt, regrassing_markdown)
#     return {"regrassing": safe_json_parse(regrassing_output)}
...
#     undertaking_prompt = load_prompt("undertaking_prompt.txt")  # Load from prompt folder
#     undertaking_output = extract_with_llm(undertaking_prompt, undertaking_markdown)
#     return {"undertaking": safe_json_parse(undertaking_output)}
...
#     western_ghat_prompt = load_prompt("western_ghat_prompt.txt")  # Load from prompt folder
#     western_ghat_output = extract_with_llm(western_ghat_prompt, western_ghat_markdown)
#     return {"western_ghat": safe_json_parse(western_ghat_output)}

# def kml_to_json(kml_path):
#     """Read KML and return JSON with coordinates in DMS format."""
#     from functions.output_generation import extract_latlon_dicts # Import here to avoid circular imports
...
#             "coordinates": coords
#         })
#     return data # Return the list directly, not json.dumps

# def verify_documents(doc_a, doc_b, doc_a_name, doc_b_name):
#     """Verify documents against each other."""
#     from llm_service import verify_with_llm # Import here to avoid circular imports
#     verify_prompt = """
...
##########################################################################################
#Chunking Added to avoid context limit issue
##########################################################################################
...
LARGE_DOCUMENT_TYPES = ['emp', 'mpa', 'dmoss', 'qlp', 'cc']
...
#     """Extract Ownership Document (OD) data."""
#     od_result, od_markdown = extract_markdown_from_file(od_file)
#     od_prompt = load_prompt("od_prompt.txt")
    
#     result = extract_with_size_check(od_prompt, od_markdown, "od")
#     return {"od": result}
...
def extract_dsr(dsr_file):
    ...
    # Load prompt and inject CAF name (EXACT SAME PATTERN)
    dsr_prompt_template = load_prompt("dsr_prompt.txt")
    dsr_prompt = dsr_prompt_template.replace("{CAF_PROPONENT_NAME}", caf_proponent)

    # Extract (EXACT SAME FLOW)
    dsr_output = extract_with_llm(dsr_prompt, dsr_markdown)
...
#     """Extract dsr Document (dsr) data."""
#     dsr_result, dsr_markdown = extract_markdown_from_file(dsr_file)
#     dsr_prompt = load_prompt("dsr_prompt.txt")
