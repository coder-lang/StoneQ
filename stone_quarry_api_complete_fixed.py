#stone_final - COMPLETE VERSION WITH ALL ENDPOINTS
...
def clear_previous_results(proposal_id: str) -> None:
    ...
    if old_zip.exists():
        try:
            os.remove(old_zip)
            logger.info(f"✅ Deleted old ZIP: {old_zip}")
        except Exception as e:
            logger.warning(f"⚠️  Could not delete {old_zip}: {e}")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            found_files = {
                ...
                "Regrassing": file_mapping.get('regrassing'),
                "Undertaking": file_mapping.get('undertaking'),
...
async def verify_process(proposal_id: str, background_tasks: BackgroundTasks = BackgroundTasks()):
    ...
    ✅ UPDATED: Clears previous verification results before starting new process.
    """
    ...
    # ✅ CRITICAL FIX: Clear previous verification results
    verification_results.pop(original_proposal_id, None)
    extraction_results.pop(original_proposal_id, None)
    blob_urls.pop(original_proposal_id, None)
...
def _process_zip_background(zip_path, original_proposal_id: str, proposal_dir, log_dir, lock_file):
    ...
    try:
        ...
        logger.info("Generating information sheet...")
        try:
            info_mapping = build_info_sheet_mapping(extracted_docs)
            filled_doc_bytes_io = fill_word_with_mapping(
                template_path=str(TEMPLATE_PATH),
                mapping=info_mapping
            )

            if filled_doc_bytes_io and filled_doc_bytes_io.getbuffer().nbytes > 0:
            ...
            logger.error(f"Information sheet generation failed: {e}", exc_info=True)
        ...
        missing_docs = [
            name for name, file_path in {
                ...
                "Regrassing": found_files.get('regrassing'),
                "Undertaking": found_files.get('undertaking'),
...
################################################################################
# ── SECTION C — SEAC ENDPOINTS + BACKGROUND TASK ─────────────────────────────
